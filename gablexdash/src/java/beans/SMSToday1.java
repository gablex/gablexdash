...
@ManagedBean
public class SMSToday1 implements Serializable {
    ...
    private BarChartModel initBarModel() {
        ...
        try {
            ...
            String sql="select  MAX(t.status) as status, MAX(s.STATUS) as statusDesc,count(*) as net from tTASKS t  left join tTASKSTATUS s on t.status = s.id where (t.status > 0 and t.msgType in (1,3) and UNIX_TIMESTAMP(t.completionDueDate) = UNIX_TIMESTAMP(curdate())) or (t.status = 0 and t.msgType in (1,3))";
