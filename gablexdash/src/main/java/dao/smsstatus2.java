 ...
 * @author kevol
 */
@ManagedBean(name = "smsstatus2")

public class smsstatus2 implements Serializable {

    private BarChartModel animatedModel2;  

 private static final String dbUserName = "user";
 private static final String dbPassword = "passwd";
   private static final String dbServer = "jdbc:mysql://Server:3306/db?useSSL=false";
...
animatedModel2.setDatatipFormat("%2$d");
   animatedModel2.setAnimate(true);
    ...
    private BarChartModel initBarModel() throws ClassNotFoundException {
        ...
        String sql3 = "select count(*) as net, status from dbSMS.tSMSOUT where (status >0 and time_submitted > '" + k + "') or status = 0 group by status";
